...
def blast(btype, query_fasta_filepath, db_namepath, evalue=E_VALUE_THRESHOLD, outfmt="\"10 qseqid sseqid evalue\"", outname=""):
    ...
    if outname is "":
        outname = l2str(query_fasta_filepath.split('/')[-1], '--', db_namepath.split('/')[-1], ext)
    execute(l2str(exe, " -query ", query_fasta_filepath, " -outfmt ", outfmt, " -evalue ", evalue, " -db " + db_namepath, " -out ", outname))
    print(l2str("Finished BLAST", btype, " of query ", query_fasta_filepath, " against database ", db_namepath))
...
def muscle(infile, outfile="", clw=False):
    ...
    comm = l2str("muscle", " -in ", infile, " -out ", outfile)
...
def parse_blast_query_results(filename):
    """
    Breaks a BLAST output outfmt=0 file into chunks of text, one query per chunk.
        - filename: the name of the BLAST output file.  Must be generated by BLAST* with outfmt=0
    """
    texts = ['']
...
def reduce_blast_output(filename):
    """
    Reduces BLAST output outfmt=0 file into query hits only  Returns the reduced text.
        - filename: the name of the BLAST output file.  Must be generated by BLAST* with outfmt=0
    """
    all_chunks = parse_blast_query_results(filename)
    texts = [all_chunks[0]]
    del all_chunks[0]
    for chunk in all_chunks:
        if 'Sequences producing significant alignments' in chunk:
            texts.append(chunk)
    return texts